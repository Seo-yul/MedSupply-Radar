...
공고 추출 정확도 실측 결과(`reports/llm/extraction_accuracy.json`): 골드 라벨 20건 전부 추출됐고(미추출 0건) macro_accuracy 100.0%다(필드별 세부 수치는 원본 참조).<!-- check: reports/llm/extraction_accuracy.json:extracted_count = 20 --><!-- check: reports/llm/extraction_accuracy.json:unextracted_count = 0 --><!-- check: reports/llm/extraction_accuracy.json:macro_accuracy = 100.0% -->
needs_review는 골드·실제 추출 둘 다 검토 필요 양성 사례가 없어 recall·precision이 정의상 계산 불가(n/a)다(tp=0·fn=0·fp=0·tn=20) — 자동확정 게이트의 과다확정(오탐) 위험은 이번 실측만으로 배제할 수 없다(§6).<!-- check: reports/llm/extraction_accuracy.json:needs_review.tp = 0 --><!-- check: reports/llm/extraction_accuracy.json:needs_review.fn = 0 --><!-- check: reports/llm/extraction_accuracy.json:needs_review.fp = 0 --><!-- check: reports/llm/extraction_accuracy.json:needs_review.tn = 20 -->
...
12. **needs_review 게이트는 양성 사례가 없어 재현·정밀을 검증하지 못했다.** 골드 라벨·실제 추출 모두 검토 필요 양성 사례가 없어(tp=0·fn=0·fp=0·tn=20) 이번 데이터로는 자동확정 게이트의 재현율·정밀도를 검증할 수 없었다(§5).<!-- check: reports/llm/extraction_accuracy.json:needs_review.tp = 0 --><!-- check: reports/llm/extraction_accuracy.json:needs_review.fn = 0 --><!-- check: reports/llm/extraction_accuracy.json:needs_review.fp = 0 --><!-- check: reports/llm/extraction_accuracy.json:needs_review.tn = 20 -->
